...
function computeMarketplacePlan({ selectedPlugins, discovered, destCommandsDir, destDroidsDir, destHooksDir, destSkillsDir }) {
  ...
  for (const p of selected) {
    ...
    // Skills (only local skills supported - remote skills require directory archiving)
    for (const s of (p.skills || [])) {
      const isUrl = /^https?:\/\//i.test(s);
      // Skip remote skills as they're not yet supported for installation
      if (isUrl) continue;
      const name = path.basename(s);
      const dest = path.join(destSkillsDir, name);
      skills.push({ plugin: p.name, name, src: s, srcType: 'local', dest, isSkill: true });
    }
  }
