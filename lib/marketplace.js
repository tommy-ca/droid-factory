...
async function githubGetRepoTree(owner, repo, ref, token, opts = {}) {
  ...
  const json = await httpGetJson(url, headers, opts);
...
async function scanPluginLocal(localDir, overrides, opts = {}) {
  ...
  if (!fs.existsSync(localDir) || !fs.statSync(localDir).isDirectory()) {
    ...
    return { commands: [], agents: [], hooks: [], errors };
  }
  ...
  debugLog(debug, 'scanPluginLocal results', { commands, agents, hooks });
...
async function scanPluginGithub(gh, overrides, opts = {}) {
  ...
  debugLog(debug, 'scanPluginGithub discovered', { commands, agents, hooks, errors });
  return { commands, agents, hooks, errors };
}
...
async function scanPluginGitlab(gl, overrides, opts = {}) {
  ...
  debugLog(debug, 'scanPluginGitlab discovered', { commands, agents, hooks, errors });
  return { commands, agents, hooks, errors };
}
...
async function discoverPlugins(marketplaceJson, context, opts = {}) {
  ...
  for (const p of normalized) {
    ...
    debugLog(debug, `Plugin ${p.name} discovered counts`, { commands: scan.commands.length, agents: scan.agents.length, hooks: scan.hooks?.length || 0, errors: scan.errors?.length || 0 });
    results.push({
      name: p.name,
      description: p.description,
      resolved,
